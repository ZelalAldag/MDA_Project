...
/* priority queue rank column */
.rank-cell { font-weight: 600; font-size: 13px; }
"""
...
def server(input, output, session):
    ...
    def map_plot():
        ...
        fig = px.scatter_mapbox(
            ...
            range_color=[-150, 150],
            size=bubble_size, zoom=11.5, height=340,
            hover_name="site_id",
            hover_data={"actual":True,"predicted":True,"signed_error":True,"lat":False,"lon":False,**hover_extra}
        )
        fig.update_layout(
    ...
    def investigation_table():
        ...
        for _, row in anomalies.iterrows():
            ...
            rows.append(f"""<tr>
              <td>{row['site_id']}</td><td>{int(row['actual'])}</td><td>{int(row['predicted'])}</td>
    ...
    def resilience_plot():
        ...
        # ── 需求2：resilience 用 20% 分位线（即bottom 20%），predicted 用 10% 分位线（top 10% = 90th pct） ──
        p80_res  = df_plot['resilience'].quantile(0.80)   # top 20% threshold
        p90_pred = df_plot['predicted'].quantile(0.90)    # top 10% threshold
        ...
        # ── 需求2：predicted 轴放大2倍（range缩小为中心±半宽/2），resilience 轴缩小为一半 ──
        x_center = (x_min + x_max) / 2
        x_half   = (x_max - x_min) / 2
        y_center = (y_min + y_max) / 2
        y_half   = (y_max - y_min) / 2

        x_range = [x_center - x_half,      x_center + x_half * 2]   # 放大range，视觉缩小为一半
        y_range = [y_center - y_half / 2,  y_center + y_half / 2]   # 缩小range，视觉放大2倍

        # ── 需求3：颜色按 resilience + predicted 加和，值越大越红 ──
    ...
    # ── Priority sites table (Top 5 by resilience + predicted) ──
    @output
    @render.ui
    def priority_table():
        res_col = 'hourly Resilience'
        ...
        def score_to_color(score):
            t = (score - score_min) / score_range  # 0~1，越高越红
            # 从 #ff9966 (浅红) 到 #cc0000 (深红)
            r1, g1, b1 = 0xff, 0x99, 0x66
            r2, g2, b2 = 0xcc, 0x00, 0x00
            r = int(r1 + (r2 - r1) * t)
            g = int(g1 + (g2 - g1) * t)
            b = int(b1 + (b2 - b1) * t)
        ...
        for i, row in top5.iterrows():
            ...
            rows.append(f"""<tr>
              <td class="rank-cell" style="color:{color}">#{rank}</td>
              <td>{row['site_id']}</td>
